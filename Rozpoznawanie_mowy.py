...
def rozpoznaj(user_input, Smieciarka, gra):
    for polecenie in re.split(r' potem | nastepnie | następnie ',user_input):
        if re.findall('odbierz|wsyp|za(l|ł)aduj|pobierz',polecenie,re.IGNORECASE):
            ...
            print(U'__Wywołano polecenie pomocy__')
            print(U'Wpisuj komendy poleceniami tekstowymi')
            print(U'Jeśli zostanie wpisane słowo kluczowe zostanie wywołane polecenie')
            print(U'Dostępne polecenia:')
            print(U'1. Wywóz śmieci: odbierz smieci z [numer smietnika] lub np. odbierz smieci z 0-20 lub odbierz smieci od 5 do 25 lub odbierz smieci z 5 6 7 8 i 9 itp.')
            print(U'2. Wyświetlanie daty, czasu')
            print(U'3. Wyjście z programu')
            print(U'4. Opróżnienie śmieciarki: wysyp smieci na smietnisku lub rozładuj smieci na smietnisku')
            print(U'5. Pomoc')
